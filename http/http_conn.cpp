...
const char *error_403_form = "You do not have permission to METHOD::GET file form this server.\n";
...
//从状态机，用于分析出一行内容
//返回值为行的读取状态，有LINE_STATUS::OK,LINE_BAD,LINE_STATUS::LINE_OPEN
...
//非阻塞ET循环读取客户数据，一次性将数据读完,直到无数据可读或对方关闭连接
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text) {
    ...
    *m_url++ = '\0';
    char *method = text;
    if (strcasecmp(method, "METHOD::GET") == 0)
        m_method = METHOD::GET;
    else if (strcasecmp(method, "METHOD::POST") == 0)
    {
